...
endpoint.handle("worker.fetch", async (args) => {
  ...
  if (op === "set") {
    await endpoint.call("host.kv.set", ["mykey", { hello: "world" }]);
  ...
  if (op === "list") {
    const entries = await endpoint.call("host.kv.list", [""]);
...
endpoint.handle("worker.fetch", async (args) => {
  ...
  if (op === "upsert") {
    await endpoint.call("host.vector.upsert", ["doc1", "hello world"]);
    return { status: 200, headers: {}, body: "upsert-ok" };
  }
  if (op === "query") {
    const results = await endpoint.call("host.vector.query", ["hello"]);
